"""Proximal Policy Optimization (PPO) implementation with CNN encoder."""
import random
import numpy as np
import torch
...
from .networks import PPOCNNNetwork


# Keep legacy FC network for checkpoint backward compat
class PPONetwork(nn.Module):
    """Legacy PPO Actor-Critic Network (flat feature vector)."""
...
class PPOAgent(BaseAgent):
    """Proximal Policy Optimization Agent with CNN encoder."""
    ...
    def __init__(self, lr=0.0003, gamma=0.99, eps_clip=0.2,
        ...
        self.model = PPOCNNNetwork(self.NUM_CHANNELS, self.ACTION_SIZE).to(self.device)
